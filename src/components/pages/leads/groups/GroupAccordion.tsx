...
export function GroupAccordion({
    ...
    const [expandedGroups, setExpandedGroups] = useState<Set<string>>(() => {
        if (groups.length > 0) {
            ...
            // ✅ Auto-expand first group and trigger fetch
            setTimeout(() => {
                onGroupExpand?.(firstGroupId, true);
            }, 100);
    ...
    // ✅ When groups change, auto-expand first group
    useEffect(() => {
    ...
    const toggleGroup = (groupId: string) => {
        ...
        // ✅ ALWAYS call onGroupExpand when toggling
        console.log(`🔄 Toggling group: ${groupId}, willBeExpanded: ${willBeExpanded}`);
        onGroupExpand?.(groupId, willBeExpanded);
    };

    const getLeadsForGroup = (group: Group): Lead[] => {
        const result = leads.filter((l) => group.leadIds.includes(l.id));
        console.log(`📋 Getting leads for group "${group.name}":`, result.length, result);
        return result;
    };
